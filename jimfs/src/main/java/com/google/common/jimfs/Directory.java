...
final class Directory extends File implements Iterable<DirectoryEntry> {
    ...
    private static final long serialVersionUID = 8982143180586565176L;
    ...
    /**
     * Creates a copy of this directory. The copy does <i>not</i> contain a copy of
    ...
    /**
     * Returns the entry linking to this directory in its parent. If this directory
     * has been deleted, this returns the entry for it in the directory it was in
     * when it was deleted.
     */
    ...
    /**
     * Returns the parent of this directory. If this directory has been deleted,
     * this returns the directory it was in when it was deleted.
     */
    ...
    @Override
    void unlinked() {
        // we don't actually remove the parent link when this directory is unlinked, but
        // the parent's
    ...
    /**
     * Returns true if this directory has no entries other than those to itself and
     * its parent.
     */
    ...
    /**
     * Returns the entry for the given name in this table or null if no such entry
     * exists.
     */
    @Nullable
     ...
     *                                  "." or if an entry already exists for the
     *                                  name
     */
     ...
     *
     * @throws IllegalArgumentException if {@code name} is a reserved name such as
     *                                  "." or no entry exists for the name
     */
    ...
    /**
     * Creates an immutable sorted snapshot of the names this directory contains,
     * excluding "." and "..".
     */
    public ImmutableSortedSet<Name> snapshot() {
        ImmutableSortedSet.Builder<Name> builder = new ImmutableSortedSet.Builder<>(
    ...
    /**
     * Checks that the given name is not "." or "..". Those names cannot be
     * set/removed by users.
     */
    ...
    private static boolean isReserved(Name name) {
        // all "." and ".." names are canonicalized to the same objects, so we can use
        // identity
        return name == Name.SELF || name == Name.PARENT;
    }

    // Simple hash table code to avoid allocation of Map.Entry objects when
    // DirectoryEntry can
    ...
    /**
     * Returns the index of the bucket in the array where an entry for the given
     * name should go.
     */
     ...
     *
     * @throws IllegalArgumentException if an entry with the given entry's name
    ...
    /**
     * Adds the given entry to the directory, overwriting an existing entry with the
     * same name if such an entry exists.
     */
    ...
    /**
     * Adds the given entry to the directory. {@code overwriteExisting} determines
     * whether an existing entry with the same name should be overwritten or an
    ...
    private void put(DirectoryEntry entry, boolean overwriteExisting) {
        ...
        entryCount++;
        if (expandIfNeeded()) {
            // if the table was expanded, the index/entry we found is no longer applicable,
            // so just add
            // the entry normally
    ...
    private static void addToBucket(int bucketIndex, DirectoryEntry[] table,
            DirectoryEntry entryToAdd) {
        DirectoryEntry prev = null;
     ...
     *
     * @throws IllegalArgumentException if there is no entry with the given name in
     *                                  the directory
     */
    @VisibleForTesting
